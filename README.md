...
| 3 | 变物性 + 长期边界 | 全周期 | 圆心含水率 ≤ 0.15 kg/kg 的烘干时间 **t_f = 57.2667 h** |
| 4 | 材料坐标收缩圆柱 | 全周期 | 半径 2.000 → 1.198 cm 后 **t_f = 50.8500 h**，净缩短 **11.2%** |
...
- **验证不是口头承诺**：守恒残差到 1e-13 量级；问题 3 / 4 有网格与时间步收敛；独立实现复算（显式差分、BDF）写在 `docs/experiments/`。
...
- **几何**：长径比 25/4 = 6.25 ≫ 1，简化为沿半径的一维轴对称问题；r = 0 单独处理对称边界，禁止把 1/r 通式直接代入圆心。
- **驱动**：附件 1 烘房温湿度分段线性插值；问题 3、4 在 t > 14400 s 后保持末值（不确定度已量化为 **+0.36 h**）。
- **问题 4**：半径按附件 2 收缩，坐标 ξ = r/R(t)，避免移动网格对流项。
...
| --- | ---: | ---: | ---: | ---: |
| 水分守恒相对误差 | 7.2e-15 | 2.8e-15 | 1.5e-14 | 5.3e-15 |
